...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://postgres:lolcat123@localhost/migmartHW4db" # DONE: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
# DONE: Set up association Table between search terms and GIFs (you can call it anything you want, we suggest 'tags' or 'search_gifs').
tags = db.Table('tags', db.Column('gifs_id',db.Integer,db.ForeignKey('gifs.id')), db.Column('searchterms_id',db.Integer,db.ForeignKey('searchterms.id')))


# DONE: Set up association Table between GIFs and collections prepared by user (you can call it anything you want. We suggest: user_collection)
user_collection = db.Table('user_collection', db.Column('gifs_id',db.Integer,db.ForeignKey('gifs.id')), db.Column('PersonalGifCollection_id',db.Integer, db.ForeignKey('PersonalGifCollection.id')))
...
class User(UserMixin, db.Model):
    ...
    gifgif = db.relationship('PersonalGifCollection', backref='User')
...
# Model to store gifs
class Gif(db.Model):
    # DONE: Add code for the Gif model such that it has the following fields:
    ...
    # DONE: Define a __repr__ method for the Gif model that shows the title and the URL of the gif
    def __repr__(self):
        return "{} (URL: {})".format(self.title, self.embedURL)

# Model to store a personal gif collection
class PersonalGifCollection(db.Model):
    # DONE: Add code for the PersonalGifCollection model such that it has the following fields:
    # id (Integer, primary key)
    ...
    # DONE: This model should have a one-to-many relationship with the User model (one user, many personal collections of gifs with different names -- say, "Happy Gif Collection" or "Sad Gif Collection")
    user_id = db.Column(db.Integer,db.ForeignKey('users.id'))
    # DONE: This model should also have a many to many relationship with the Gif model (one gif might be in many personal collections, one personal collection could have many gifs in it).
...
class SearchTerm(db.Model):
    # DONE: Add code for the SearchTerm model such that it has the following fields:
    # id (Integer, primary key)
    ...
    # DONE: Define a __repr__ method for this model class that returns the term string
    def __repr__(self):
        return "Search Term entered: {}".format(self.term)
...
def get_gifs_from_giphy(search_string):
    ...
    # DONE: This function should make a request to the Giphy API using the input search_string, and your api_key (imported at the top of this file)
    ...
    data = json.loads(requests.get(url=baseurl,params=params).text)
...
def get_or_create_gif(title, url):
    ...
    # DONE: This function should get or create a Gif instance. Determining whether the gif already exists in the database should be based on the gif's title.
    gif_check = db.session.query(Gif).filter_by(title=title).first()
    if gif_check:
        return gif_check
    else:
        gif_check = Gif(title=title,embedURL=url)
        db.session.add(gif_check)
        db.session.commit()
        return gif_check

def get_or_create_search_term(term):
    """Always returns a SearchTerm instance"""
    # DONE: This function should return the search term instance if it already exists.
    ...
    term_check = db.session.query(SearchTerm).filter_by(term=term).first()
    if term_check:
        return term_check
    else:
        term_check = SearchTerm(term=term)
        gif_list = get_gifs_from_giphy(term_check)

        for g in gif_list['data']:
            g = get_or_create_gif(g['title'],g['embed_url'])
            term_check.gifs.append(g)

        db.session.add(term_check)
        db.session.commit()
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    # DONE: This function should get or create a personal gif collection. Uniqueness of the gif collection should be determined by the name of the collection and the id of the logged in user.
    ...
    collection_check = db.session.query(PersonalGifCollection).filter_by(name=name,user_id=current_user.id).first()
    if collection_check:
        return collection_check
    else:
        collection_check = PersonalGifCollection(name=name,user_id=current_user.id,gifs=[])

        for g in gif_list:
            collection_check.gifs.append(g)

        db.session.add(collection_check)
        db.session.commit()
        return collection_check
...
def index():
    # DONE: Edit this view function, which has a provided return statement, so that the GifSearchForm can be rendered.
...
def search_terms():
    # DONE: Edit this view function so it renders search_terms.html.
    ...
    terms_list = SearchTerm.query.all()
    return render_template('search_terms.html',all_terms=terms_list)
...
@login_required
def collections():
    # DONE: This view function should render the collections.html template so that only the current user's personal gif collection links will render in that template. Make sure to examine the template so that you send it the correct data!
    gif_collection = db.session.query(PersonalGifCollection).filter_by(user_id=current_user.id)

    return render_template('collections.html',collections=gif_collection)
